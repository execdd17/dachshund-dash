...
- **Global**: Firebase Firestore `scores` collection (`firebase.js` + `leaderboard/global.js`), **per difficulty**: every submission carries the session's difficulty label (`state.difficulty`, e.g. `'NORMAL'`), the UI (`leaderboard/ui.js`) shows one board per difficulty with tabs to switch (defaults to NORMAL; jumps to the player's chosen difficulty after setup), and `qualifies(score, difficulty)` judges against that difficulty's board only. Fetch is a single mixed query (limit `GLOBAL_MAX_SCORES × #difficulties`) bucketed client-side by the pure `getScoresForDifficulty`; legacy docs without a `difficulty` field are filtered out entirely. On death, `killDog` stores the run's global rank (`store.placement` → pure `getGlobalPlacement`, judged against the last-fetched board for the session difficulty, ties below existing entries) in `state.globalPlacement`; when set, `drawGameOverScreen` shows a gold "You got Nth place on the global leaderboard!" banner above the GAME OVER box (ordinal via `formatOrdinal`). Null (board not loaded or didn't make the top `GLOBAL_MAX_SCORES`) means no banner. Submissions carry a simple non-cryptographic checksum (`checksum.js`, DJB2-style salted hash over name|score|timestamp|difficulty) as friction against casual tampering — not real security.

...
- `state.gameState` is the state machine: `idle` → `running` → `dead`/`enteringName` → back to `running`.
...
- **New cosmetic item**: drop the PNG under `png/<slot>/…` and add an entry to `COSMETIC_DEFS` in `cosmetics/cosmetics.js`; per-slot placement is `SLOT_ANCHOR_OFFSET` + the pose-calibrated `HEAD_ANCHORS`.
